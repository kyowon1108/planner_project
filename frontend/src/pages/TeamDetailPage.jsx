...
const TeamDetailPage = () => {
  ...
  const [roleChangeAnchorEl, setRoleChangeAnchorEl] = useState(null);
  const [selectedMemberForRoleChange, setSelectedMemberForRoleChange] = useState(null);
  ...
  const fetchTeam = useCallback(async () => {
    ...
    try {
      ...
      setPendingInvites(invitesData.filter((invite) => !invite.is_used));
      setLoading(false);
    } catch (err) {
  ...
  const handleRoleChange = async (newRole) => {
    ...
    try {
      await teamAPI.updateTeamMemberRole(parseInt(id!), selectedMemberForRoleChange, { role });
  ...
  const getMemberCounts = () => {
    if (!team?.members) return { total, owner, admin, manager, editor, viewer, member, guest };
    ...
    return {
      ...
      viewer.members.filter(m => m.role === 'viewer').length,
              member.members.filter(m => m.role === 'editor').length,
  ...
  const getRolePriority = (role)=> {
    ...
    return priorities[role typeof priorities] || 0;
  };

  // 현재 사용자가 변경할 수 있는 역할인지 확인
  const canChangeRole = (targetMemberRole)=> {
                ...
                >
                  <CardContent sx={{ p }}>
                    ...
                    <Box sx={{ 
                      ...
                      <Typography variant="subtitle2" sx={{ mb, fontWeight, color ? '#ffffff' : '#2c3e50' }}>
                        멤버 검색 및 필터
                      </Typography>
                      ...
                      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <ToggleButtonGroup
                          ...
                          sx={{ borderRadius }}
                        >
                          <ToggleButton value="asc" sx={{ borderRadius }}>
                            <ArrowUpwardIcon />
                          </ToggleButton>
                          <ToggleButton value="desc" sx={{ borderRadius }}>
                            <ArrowDownwardIcon />
                          </ToggleButton>
                        </ToggleButtonGroup>
                      </Box>
      ...
      >
        ...
        <DialogContent sx={{ pt }}>
          ...
          <FormControl fullWidth>
            <InputLabel sx={{ color ? 'text.primary' : 'text.secondary' }}>
              새 소유자
            </InputLabel>
            <Select
              ...
              {team.members?.filter(m => m.role !== 'owner').map(member => (
                <MenuItem 
                  ...
                  sx={{
                    color ? '#ffffff' : '#2c3e50',
                    '&:hover': {
                      backgroundColor ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.04)',
                    },
                  }}
                >
