...
import QuickDateSelect from '../components/QuickDateSelect.jsx';
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'priority':
          ...
          const aPriority = priorityOrder[a.priority typeof priorityOrder] ?? 2;
          const bPriority = priorityOrder[b.priority typeof priorityOrder] ?? 2;
  ...
  const handleAddTodo = async () => {
    ...
    try {
      ...
      const todoData = {
        ...
        due_date,
        assigned_to.assigned_to.length > 0 ? todoFormData.assigned_to : null,
      };
  ...
  const handleUpdateTodo = async () => {
    ...
    try {
      ...
      const todoData = {
        title.title,
        description.description || null,
        priority.priority, // 이미 영어 값이므로 그대로 사용
        due_date,
        assigned_to.assigned_to.length > 0 ? todoFormData.assigned_to : null,
      };
                ...
                >
                  <Box sx={{ p }}>
                    <Box display="flex" alignItems="center" mb={3}>
                      <AssignmentIcon sx={{ mr, color: 'primary.main', fontSize }} />
                      <Typography 
                        variant="h6"
                        sx={{
                          fontWeight,
                    ...
                    <Box display="flex" alignItems="center" gap={1} mb={2}>
                      <ScheduleIcon sx={{ fontSize, color ? '#b0b0b0' : 'text.secondary' }} />
                      ...
                      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                        ...
                        <Button
                          ...
                          sx={{ 
                            ...
                            fontSize: '0.8rem',
                            borderRadius,
                            borderColor === 'all' ? 'grey.300' : 
                                        statusFilter === 'pending' ? 'green.main' : 'red.main',
                            color === 'all' ? 'text.primary' : 
                                   statusFilter === 'pending' ? 'green.main' : 'red.main',
                            '&:hover': {
                              borderColor === 'all' ? 'grey.400' : 
                                          statusFilter === 'pending' ? 'green.dark' : 'red.dark',
                              backgroundColor === 'all' ? 'grey.50' : 
                              ...
                              <Box display="flex" alignItems="center" gap={1}>
                                <Typography
                                  ...
                                  sx={{
                                    ...
                                    color.is_completed ? 'text.secondary' : 'text.primary',
                                  }}
                                >
                                  {todo.title}
                                </Typography>
          ...
          <Dialog open={addTodoDialogOpen} onClose={() => setAddTodoDialogOpen(false)} maxWidth="sm" fullWidth>
            ...
            <DialogContent>
              ...
              <TextField
                ...
                onChange={(e) => setTodoFormData({ ...todoFormData, title.target.value })}
                sx={{ mb }}
              />
              <TextField
                margin="dense"
                ...
                onChange={(e) => setTodoFormData({ ...todoFormData, description.target.value })}
                sx={{ mb }}
              />
              <FormControl fullWidth sx={{ mb }}>
                ...
                <Select
                  ...
                  label="우선순위"
                  onChange={(e) => setTodoFormData({ ...todoFormData, priority.target.value })}
                >
              ...
              <Box sx={{ mb }}>
                <TextField
                  ...
                  onChange={(e) => setTodoFormData({ ...todoFormData, due_date.target.value })}
                  fullWidth
                  InputLabelProps={{ shrink }}
                  sx={{ mb }}
                />
                <QuickDateSelect
                  onDateSelect={(date) => setTodoFormData({ ...todoFormData, due_date })}
                />
              </Box>
                  {/* 담당자 선택 (플래너가 선택된 경우에만 표시) */}
                  {planner?.team_id && teamMembers.length > 0 && hasPermission('todo_assign') && (
                    <Box sx={{ mb }}>
                      <Typography variant="subtitle2" sx={{ mb, fontWeight, color: 'text.primary' }}>
                        담당자 선택
                      </Typography>
                            ...
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                              {(selected[]).map((userId) => {
                                ...
                                return (
                                  <Chip 
                                    key={userId} 
                                    label={member ? (member).user?.name || `사용자 ${userId}` : `사용자 ${userId}`}
                                    size="small"
                                  />
                                );
                              })}
                            ...
                            <MenuItem key={member.id} value={member.user_id}>
                              <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                <Typography variant="body2">
                                  {(member).user?.name || `사용자 ${member.user_id}`} ({(member).role})
                                </Typography>
                              </Box>
                            </MenuItem>
                          ))}
