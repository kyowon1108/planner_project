...
const TodosPage = () => {
  ...
  const [newTodoPlannerId, setNewTodoPlannerId] = useState('');
  ...
  // 플래너 선택 시 팀 정보 가져오기
  const handlePlannerChange = async (plannerId) => {
  ...
  const handleCreateTodo = async () => {
    ...
    const dueDateTime = newTodoDueDate ? `${newTodoDueDate}T00:00` : null;
    ...
    try {
      await todoAPI.createTodo({
        ...
        due_date,
        planner_id,
        assigned_to.length > 0 ? newTodoAssignedTo : null,
      });
