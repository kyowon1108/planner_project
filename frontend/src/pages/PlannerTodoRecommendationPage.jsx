...
const PlannerTodoRecommendationPage = () => {
  ...
  if (error) return (
    <Alert severity="error" sx={{ m }}>
      {error}
      <Button onClick={() => navigate(`/planners/${state?.plannerId}`)} sx={{ ml }}>
        플래너로 이동
      </Button>
    </Alert>
  );
          ...
          <Slide direction="up" in={animateIn} timeout={800}>
            <Fade in={animateIn} timeout={1000}>
              ...
              >
                <CardContent sx={{ p }}>
                  <Box sx={{ mb }}>
                    <Typography variant="h5" component="h2" sx={{ mb, fontWeight }}>
                  ...
                  <Alert severity="info" sx={{ mb, borderRadius }}>
                  ...
                  <List sx={{ p }}>
                    {recommendedTodos.map((todo, index) => (
                         ...
                         sx={{
                           ...
                           borderRadius,
                           mb,
                           backgroundColor.has(index) ? 'primary.light' : 'transparent',
                           '&:hover': {
                             backgroundColor.has(index) ? 'primary.light' : 'action.hover',
                           },
                           cursor: 'pointer',
                         }}
                  ...
                  {recommendedTodos.length === 0 && (
                    <Box sx={{ textAlign: 'center', py }}>
                      ...
                      <Typography variant="h6" color="text.secondary" sx={{ mb }}>
                        추천할 할일이 없습니다
                      </Typography>
