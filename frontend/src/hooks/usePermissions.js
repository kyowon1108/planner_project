...
/**
 * @typedef {'team_create'|'team_update'|'team_delete'|'team_invite'|'team_remove_owner'|'team_remove_admin'|'team_remove_member'|'team_view_analytics'|'planner_create'|'planner_update'|'planner_delete'|'planner_assign'|'planner_view'|'planner_approve'|'todo_create'|'todo_update'|'todo_delete'|'todo_assign'|'todo_view'|'todo_complete'|'post_create'|'post_update'|'post_delete'|'post_view'|'post_approve'} Permission
 */

/**
 * @typedef {'owner'|'admin'|'manager'|'editor'|'viewer'|'guest'} Role
 */
...
export const usePermissions = (teamId) => {
  const { user } = useAuth();
  ...
  const canEditContent = (contentCreatorId) => {
