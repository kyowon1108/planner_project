...
const NotificationBar = ({ onInviteAccepted }) => {
  const { user } = useAuth();
  ...
  const [currentToastNotification, setCurrentToastNotification] = useState(null);
  ...
  const handleInviteAction = async (notification, action: 'accept' | 'reject') => {
    try {
      ...
      if (action === 'accept') {
        ...
        if (notification.related_id) {
          ...
          try {
            ...
            const invite = pendingInvites.find((inv) => inv.team_id === notification.related_id);
  ...
  const handleToastAction = async (action, data) => {
