...
// 날짜가 임박했는지 확인 (3일 이내)
export const isDeadlineImminent = (dueDate) => {
...
// 마감일 텍스트 생성
export const getDeadlineText = (dueDate) => {
  if (!dueDate) return '';
...
// 상세 마감일 텍스트 생성 (날짜만)
export const getDetailedDeadlineText = (dueDate) => {
  if (!dueDate) return '';
 ...
 * @param {string} dueDate 마감일시 (KST ISO 문자열)
 * @returns {string} 간단한 마감시간 텍스트
 */
export const getSimpleDeadlineText = (dueDate) => {
  const deadline = new Date(dueDate);
  const now = new Date();
